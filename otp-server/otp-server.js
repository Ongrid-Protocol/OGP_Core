...
// Default canister ID
const DEFAULT_CANISTER_ID = process.env.CANISTER_ID || 'your-default-canister-id';
...
function getOtpUsingDfx(peerId, canisterId = DEFAULT_CANISTER_ID) {
  const command = `dfx canister call ${canisterId} generate_otp "(\\\"${peerId}\\\")"`;
...
function associateDetailsWithOtp(otp, details, canisterId = DEFAULT_CANISTER_ID) {
  ...
  let command = `dfx canister call ${canisterId} associate_node_details_with_otp "(record { `;
...
function isOtpUsed(otp, canisterId = DEFAULT_CANISTER_ID) {
  const command = `dfx canister call ${canisterId} is_otp_used "(\\\"${otp}\\\")"`;
  console.log(`Executing command: ${command}`);
